export default {
  mount: {
    title: "网盘管理",
    permissionRequired: "需要管理员权限或有效的API密钥才能访问网盘管理，请",
    loginAuth: "登录管理面板或配置API密钥",
    cancel: "取消",
    backToFileList: "返回文件列表",
    noPermissionForPath: "您没有权限访问此目录的内容。您只能访问 {path} 及其子目录。",
    batchDelete: {
      title: "确认批量删除",
      message: "您确定要删除选中的 {count} 个项目吗？此操作不可撤销。",
      selectedItems: "选中的项目:",
      folder: "(文件夹)",
      moreItems: "... 等 {count} 个项目",
      confirmButton: "删除",
      cancelButton: "取消",
      deleting: "删除中...",
    },
    operations: {
      upload: "上传",
      createFolder: "新建文件夹",
      createTextFile: "新建文本",
      refresh: "刷新",
      refreshing: "刷新中...",
      viewMode: "视图模式",
      batchOperations: "批量操作",
      tasks: "任务列表",
    },
    createFolder: {
      enterName: "请输入文件夹名称",
      folderName: "文件夹名称",
      placeholder: "新文件夹",
      cancel: "取消",
      create: "创建",
      creating: "创建中...",
    },
    createTextFile: {
      title: "新建文本",
      enterName: "请输入文本文件名称，内容将以 UTF-8 文本保存",
      fileName: "文件名",
      placeholder: "新建文本.txt",
      defaultName: "新建文本.txt",
      cancel: "取消",
      create: "创建",
      creating: "创建中...",
    },
    viewModes: {
      list: "列表视图",
      grid: "网格视图",
      gallery: "图廊视图",
    },
    // 设置抽屉
    settings: {
      title: "本地设置",
      viewMode: "视图模式",
      listView: "列表",
      gridView: "网格",
      galleryView: "图廊",
      density: "显示密度",
      densityCompact: "紧凑",
      densityComfortable: "舒适",
      densitySpacious: "宽松",
      displayOptions: "显示选项",
      showCheckboxes: "显示勾选框",
      sorting: "排序设置",
      sortBy: "排序方式",
      sortByName: "按名称",
      sortBySize: "按大小",
      sortByModified: "按修改时间",
      sortByType: "按类型",
      sortOrder: "排序顺序",
      ascending: "升序",
      descending: "降序",
      foldersFirst: "文件夹优先",
      animations: "动画效果",
      enableAnimations: "启用动画",
      showActionButtons: "显示操作按钮",
      itemSpacing: "文件间距",
      spacingCompact: "紧凑",
      spacingStandard: "标准",
      spacingRelaxed: "宽松",
      fileNameDisplay: "文件名显示",
      fileNameEllipsis: "省略号截断",
      fileNameScroll: "悬停滚动",
      fileNameWrap: "自动换行",
      resetToDefault: "恢复默认设置",
    },
    fileList: {
      loading: "加载中...",
      empty: "此目录为空",
      noMountPoints: "无可用挂载点",
      name: "名称",
      size: "大小",
      modifiedTime: "修改时间",
      type: "类型",
      actions: "操作",
      selectAll: "全选",
      deselectAll: "取消全选",
      clickToSort: "点击排序",
      loadMore: "加载更多",
      pagedHint: "如果这个目录文件很多，系统会先显示一部分；往下滑会自动继续加载，也可以点“加载更多”。",
    },
    rename: {
      title: "重命名",
      enterNewName: "请输入新的名称",
      newName: "新名称",
      cancel: "取消",
      confirm: "确认",
      renaming: "正在重命名...",
      failed: "重命名失败",
    },
    delete: {
      title: "确认删除",
      message: "您确定要删除{type} {name} 吗？",
      folderWarning: "此操作将删除文件夹及其所有内容。",
      cancel: "取消",
      confirm: "删除",
      deleting: "删除中...",
    },
    messages: {
      apiKeyInfoUpdated: "已更新API密钥信息",
      refreshSuccess: "刷新成功",
      refreshFailed: "刷新失败，请重试",
      refreshStillRunning: "目录仍在刷新中，当前列表会保留显示，请稍候...",
      getDirectoryContentFailed: "获取目录内容失败: {message}",
      getDirectoryContentFailedUnknown: "获取目录内容失败: {message}",
      fileUploadSuccess: "文件上传成功",
      fileUploadFailed: "文件上传失败: {message}",
      fileUploadFailedUnknown: "文件上传失败: {message}",
      uploadCancelling: "正在取消上传...",
      folderCreateSuccess: "文件夹创建成功",
      createFolderSuccess: "文件夹创建成功",
      createTextFileSuccess: "文本文件创建成功",
      createTextFileFailed: "文本文件创建失败",
      itemAlreadyExists: "{name} 已存在",
      folderCreateFailed: "文件夹创建失败: {message}",
      folderCreateFailedUnknown: "文件夹创建失败: {message}",
      renameSuccess: "{type}重命名成功",
      renameFailed: "重命名失败: {message}",
      renameFailedUnknown: "重命名失败: {message}",
      deleteSuccess: "{type}删除成功",
      deleteFailed: "删除失败: {message}",
      deleteFailedUnknown: "删除失败: {message}",
      downloadPreparing: "准备下载文件...",
      downloadSuccess: "文件下载成功",
      downloadStarted: "开始下载 {name}",
      downloadFailed: "文件下载失败: {message}",
      downloadFailedUnknown: "文件下载失败: {message}",
      uploading: "正在上传 {name}...",
      initializationFailed: "初始化失败: {message}",
      batchDeleteFailed: "批量删除失败: {message}",
      previewError: "文件预览加载失败",
      batchDeleteSuccess: "成功删除了 {count} 个项目",
      batchDeletePartial: "批量删除部分成功：成功 {success} 个，失败 {failed} 个",
      copySuccess: "{message}",
      copyFailed: "复制失败: {message}",
      getFileLinkFailed: "获取文件直链失败: {message}",
      getFileLinkError: "获取文件直链错误: {message}",
      noItemsSelected: "请选择要操作的项目",
      uploadSuccess: "上传成功",
      uploadSuccessWithCount: "成功上传 {count} 个文件",
      uploadSuccessWithSkipped: "上传完成：{count} 个文件，其中 {skipped} 个内容已存在，已跳过上传（秒传）",
      uploadFailed: "上传失败: {message}",
      shareCreated: "分享链接已创建并复制到剪贴板：{url}",
      shareCreateFailed: "创建分享链接失败: {message}",
      pageLoadFailed: "页面加载失败",
      pageLoadFailedRetry: "页面加载失败，请刷新重试",
      previewLoadFailed: "预览加载失败",
      fileSaveSuccess: "文件 {name} 保存成功",
      fileSaveFailed: "文件保存失败",
      linkCopiedSuccess: "文件直链已复制到剪贴板",
      pathCopiedSuccess: "路径已复制到剪贴板",
      nameCopiedSuccess: "名称已复制到剪贴板",
      zipPreparing: "正在准备 ZIP 下载任务...",
      noFilesForZip: "没有可打包的文件",
      zipCreateFailed: "创建 ZIP 下载任务失败: {message}",
      itemProcessingLocked: "该文件或文件夹正在批量任务处理中，暂不能重命名、编辑、移动或删除",
    },
    filePreview: {
      downloadFile: "下载文件",
      directPreview: "直链预览",
      generating: "生成中...",
      createShare: "生成分享链接",
      creatingShare: "创建中...",
      fileSize: "文件大小:",
      modifiedTime: "修改时间:",
      fileType: "文件类型:",
      unknown: "未知",
      editMode: "编辑模式",
      previewMode: "预览模式",
      saving: "保存中...",
      save: "保存",
      cancel: "取消",
      language: "语言:",
      autoDetect: "自动检测",
      configFile: "配置文件",
      loadingPreview: "加载中...",
      previewError: "加载文件预览失败",
      retryLoad: "请尝试重新加载或下载文件查看",
      retry: "重试",
      cannotPreview: "文件无法预览",
      downloadToView: "当前文件类型不支持在线预览，请点击下载按钮下载查看",
      wordPreview: "Word文档预览",
      excelPreview: "Excel表格预览",
      powerpointPreview: "PowerPoint演示文稿预览",
      exitFullscreen: "退出全屏",
      fullscreen: "全屏",
      browserNative: "原生预览",
      pdfjsLabel: "PDF.js",
      officeProvider: {
        native: "本地预览",
        microsoft: "Microsoft Office Online",
        google: "Google Docs",
      },
      nativeRenderFailed: "本地预览失败",
      tryExternalProvider: "请尝试切换到 Microsoft 或 Google 预览",
      officePreview: "Office预览",
      useMicrosoftPreview: "使用Microsoft预览",
      useGooglePreview: "使用Google预览",
      htmlPreview: "HTML预览",
      browserNotSupport: "您的浏览器不支持",
      videoTag: "视频标签",
      audioTag: "音频标签",
      cannotLoadText: "无法加载文本内容",
      loadTextError: "加载文本内容时出错",
      pdfLoading: "加载中...",
      noPdfPreview: "无法加载 PDF 预览",
      officeLoading: "加载中...",
      getStorageLinkFailed: "获取存储直链失败: {message}",
      storagePreviewFailed: "存储直链预览失败: {message}",
      fileTooLarge: "文件内容过大，超过最大限制(10MB)。请减少文件大小后重试。",
      saveFileFailed: "保存文件失败: {message}",
      saveFileError: "保存文件时发生错误: {message}",
      fileContentTooLarge: "文件内容过大，超过服务器限制",
      noPermissionUpdate: "没有权限更新该文件",
      serverNoResponse: "服务器无响应，请检查网络连接",
      saveFileShortcut: "保存文件 (Ctrl+S)",
      editNotAllowedForType: "当前文件类型未启用在线编辑",
      textTooLargeTitle: "文本太大",
      textTooLargeMessage: "该文本文件大小为 {size}，超过 {limit} 限制，不支持在线预览和编辑。请下载后在本地查看。",
      textTooLargeEditNotAllowed: "文本文件超过 {limit}，不支持在线编辑和预览",
      unsavedChangesTitle: "编辑内容尚未保存",
      unsavedChangesMessage: "当前文件有未保存的修改，返回文件列表前是否保存？",
      saveAndBack: "保存并返回",
      discardAndBack: "不保存返回",
      keepEditing: "继续编辑",
      rightClickHint: "右键编辑器查看更多功能（插入时间戳、文本统计、格式化等）",
      cannotGetEditorContent: "无法获取编辑器内容",
      textPreview: "文本",
      codePreview: "代码",
      imagePreview: "图片",
      videoPreview: "视频",
      audioPreview: "音频",
      iframePreview: "Iframe",
      archivePreview: "压缩包",
      previewTypeOther: "预览",
    },
    audioPreview: {
      loadingAudio: "加载中...",
      audioPlayer: "音频播放器",
      sendToGlobalPlayer: "发送到全局播放器",
      playingInGlobalPlayer: "正在全局播放器中播放",
      backToPreview: "返回",
      closePlayer: "关闭播放器",
      expandPlayer: "展开播放器",
      collapsePlayer: "收起播放器",
      unknownAudio: "未知音频",
      unknownArtist: "未知艺术家",
    },
    videoPreview: {
      loadingVideo: "加载中...",
      videoPlayer: "视频播放器",
      prevVideo: "上一集",
      nextVideo: "下一集",
      playlist: "播放列表",
      showPlaylist: "显示播放列表",
    },
    textPreview: {
      loadingText: "加载中...",
    },

    taskList: {
      title: "任务列表",
      loading: "加载任务中...",
      empty: "暂无任务",
      status: {
        pending: "待处理",
        failed: "失败",
        cancelled: "已取消",
      },
      cancel: "取消",
      itemsFailed: "项失败",
      unknownFile: "未知文件",
      viewFullManagement: "查看完整任务管理",
      retry: {
        retryCount: "已重试 {count} 次",
        withRetry: "(重试 {count} 次)",
      },
    },
    taskManager: {
      title: "任务管理",
      noTasks: "暂无任务",
      noTasksDescription: "当前没有正在进行或已完成的任务",
      activeTasks: "正在进行的任务 ({count})",
      completedTasks: "已完成的任务 ({count})",
      clearCompleted: "清除已完成",
      cancel: "取消",
      retry: "重试",
      details: "详情",
      hideDetails: "隐藏详情",
      status: {
        pending: "待处理",
        running: "运行中",
        completed: "已完成",
        failed: "失败",
        cancelled: "已取消",
      },
      types: {
        copy: "复制",
        move: "移动",
        upload: "上传",
        delete: "删除",
        download: "下载",
      },
      progress: "进度: {current}/{total}",
      timeElapsed: "耗时: {time}",
      createdAt: "创建时间: {time}",
      updatedAt: "更新时间: {time}",
      error: "错误: {message}",
      confirmCancel: "确定要取消这个任务吗？",
      confirmClearCompleted: "确定要清除所有已完成的任务吗？",
      downloading: "下载中",
      uploading: "上传中",
      processing: "处理中",
      currentFile: "当前文件: {fileName}",
      processed: "已处理: {current}/{total}",
      completedAt: "完成时间: {time}",
      processedItems: "处理项目: {current}/{total}",
      success: "成功: {count}",
      failed: "失败: {count}",
      skipped: "跳过: {count}",
      partialComplete: "部分完成",
      copyTask: "复制任务",
      moveTask: "移动任务",
      uploadTask: "上传任务",
      deleteTask: "删除任务",
      downloadTask: "下载任务",
      unknownTask: "未知任务",
      waiting: "等待中",
      unknown: "未知",
      unknownTime: "未知时间",
      copyTaskName: "复制 {count} 个项目到 {path}",
      moveTaskName: "移动 {count} 个项目到 {path}",
      copyStarted: "开始复制 {count} 个项目到 {path}，可在任务管理中查看进度",
      moveStarted: "开始移动 {count} 个项目到 {path}，可在任务管理中查看进度",
      taskCompletedRefresh: "任务已完成，目录已刷新",
      // 文件状态
      fileStatus: {
        pending: "等待",
        downloading: "下载中",
        completed: "已完成",
        failed: "失败",
        preparing: "准备中...",
      },
      // 文件进度
      fileProgress: "文件进度",
      filesCount: "个文件",
    },
    copyModal: {
      title: "选择目标文件夹",
      selectedInfo: "已选择: {count} 个项目 ({folders} 个文件夹, {files} 个文件)",
      targetLocation: "目标位置:",
      loading: "加载中...",
      rootDirectory: "根目录",
      cancel: "取消",
      confirm: "确认复制",
      confirmCopy: "确认复制",
      submitting: "复制中...",
      copying: "复制中...",
      skipExisting: "跳过已存在的文件",
      skipExistingTooltip: "如果目标位置已存在同名文件，则跳过复制",
      warnings: {
        recursiveCopy: "警告：不能将文件夹复制到其自身或其子目录中，这可能导致无限递归。",
        selfCopy: "警告：不能将文件夹复制到其自身。",
      },
      warningTitle: "复制警告",
      confirmPotentialIssue: "检测到潜在问题：{warning}\n\n是否仍要继续复制？",
      continue: "继续复制",
      createFailed: "创建复制作业失败",
      failed: "复制启动失败",
      createFolder: "新建文件夹",
      createFolderDescription: "将在目标目录 {path} 下新建文件夹，并自动选中为目标目录",
      folderName: "文件夹名称",
      folderPlaceholder: "新文件夹",
      createFolderConfirm: "创建",
      creatingFolder: "创建中...",
      createFolderFailed: "创建文件夹失败",
    },
    moveModal: {
      title: "选择移动目标文件夹",
      selectedInfo: "已选择: {count} 个项目 ({folders} 个文件夹, {files} 个文件)",
      targetLocation: "目标位置:",
      loading: "加载中...",
      rootDirectory: "根目录",
      cancel: "取消",
      confirm: "确认移动",
      submitting: "移动中...",
      warnings: {
        recursiveMove: "不能将文件夹移动到其自身或其子目录中。",
        selfMove: "不能将文件夹移动到其自身。",
        sameLocation: "目标位置与当前位置相同，无需移动。",
      },
      warningTitle: "移动警告",
      confirmPotentialIssue: "检测到问题：{warning}",
      continue: "继续移动",
      createFailed: "创建移动作业失败",
      failed: "移动启动失败",
      createFolder: "新建文件夹",
      createFolderDescription: "将在目标目录 {path} 下新建文件夹，并自动选中为目标目录",
      folderName: "文件夹名称",
      folderPlaceholder: "新文件夹",
      createFolderConfirm: "创建",
      creatingFolder: "创建中...",
      createFolderFailed: "创建文件夹失败",
    },
    linkCopied: "文件直链已复制到剪贴板",
    fileItem: {
      download: "下载",
      getLink: "获取直链",
      edit: "编辑",
      rename: "重命名",
      delete: "删除",
      processing: "处理中",
      preview: "预览",
      copy: "复制",
      move: "移动",
      properties: "属性",
    },
    contextMenu: {
      open: "打开",
      addToBasket: "添加到文件篮",
      batchDownload: "批量下载",
      downloadZip: "打包下载 ZIP",
      batchDownloadZip: "批量下载 ZIP",
      batchCopy: "批量复制",
      batchMove: "批量移动",
      batchAddToBasket: "批量添加到文件篮",
      batchDelete: "批量删除",
      copyPath: "复制路径",
      copyName: "复制名称",
      properties: "属性",
      refresh: "刷新",
      showCheckboxes: "显示勾选框",
      hideCheckboxes: "隐藏勾选框",
      zipTaskName: "ZIP 下载任务（{count} 个文件）",
    },
    properties: {
      basic: "基本信息",
      selection: "选择摘要",
      actions: "操作",
      type: "类型",
      size: "大小",
      modified: "修改时间",
      path: "路径",
      source: "来源",
      totalItems: "项目数",
      files: "文件数",
      folders: "文件夹数",
      batchTitle: "已选择 {count} 个项目",
      batchSubtitle: "{files} 个文件，{folders} 个文件夹",
      multipleTypes: "多个类型",
    },
    toolbar: {
      more: "更多操作",
      collapse: "收起",
      refresh: "刷新",
      newFolder: "新建文件夹",
      newTextFile: "新建文本",
      upload: "上传文件",
      showCheckboxes: "显示勾选框",
      hideCheckboxes: "隐藏勾选框",
      tasks: "任务列表",
      settings: "设置",
    },
    pathPassword: {
      title: "需要密码",
      description: "目录 {path} 需要密码才能访问",
      label: "请输入密码",
      placeholder: "输入密码",
      verify: "验证",
      cancel: "取消",
      back: "返回",
      verifying: "验证中...",
      incorrectPassword: "密码错误，请重试",
      verifyFailed: "密码验证失败，请重试",
      verified: "密码验证成功",
    },
    fileTypes: {
      folder: "文件夹",
      file: "文件",
      image: "图片",
      video: "视频",
      audio: "音频",
      document: "文档",
      archive: "压缩包",
      code: "代码",
      unknown: "未知类型",
    },
    sizeUnits: {
      bytes: "字节",
      kb: "KB",
      mb: "MB",
      gb: "GB",
      tb: "TB",
    },
    uppy: {
      title: "文件上传",
      uploadMethod: "上传方式",
      presignedUpload: "预签名上传",
      streamUpload: "流式上传",
      formUpload: "表单上传",
      multipartUpload: "分片上传",
      resumeSupport: "断点续传",
      presignedMode: "预签名模式",
      streamMode: "流式模式",
      formMode: "表单模式",
      multipartMode: "分片模式",
      presignedModeTooltip: "使用预签名URL，安全快速",
      streamModeTooltip: "通过后端流式中转上传到存储",
      formModeTooltip: "通过表单(form-data)上传到存储",
      multipartModeTooltip: "适合大文件，支持断点续传",
      advancedFeatures: "高级功能",
      enabledCount: "{count} 个已启用",
      targetPath: "目标路径:",
      startUpload: "开始上传",
      uploading: "上传中...",
      pasteSupport: "支持",
      pasteKey: "Ctrl+V",
      pasteHint: "粘贴剪贴板中的文件",
      initializationFailed: "上传组件初始化失败: {message}",
      close: "关闭",
      fileName: "文件名",
      customFilename: "自定义文件名（可选）",
    },
  },
  breadcrumb: {
    navigation: "面包屑导航",
    root: "根目录",
    exitSelection: "退出勾选", // 仅保留此项，用于 FloatingActionBar
  },
};
